...
function activate(context) {
    ...
    // 注册文档格式化提供程序
    const formatProvider = vscode.languages.registerDocumentFormattingEditProvider('shellscript', {
        provideDocumentFormattingEdits(document, options, token) {
            return formatDocument(document, options, token);
        }
    });
    // 注册 Code Actions 提供程序
    const codeActionProvider = vscode.languages.registerCodeActionsProvider('shellscript', new ShellFormatCodeActionProvider(), {
    ...
    // 注册命令
    const formatCommand = vscode.commands.registerCommand('shellformat.formatDocument', async () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            await formatDocument(editor.document);
        }
    });
    const fixAllCommand = vscode.commands.registerCommand('shellformat.fixAllProblems', async (uri) => {
        let document;
        if (uri) {
            // 从问题面板的修复命令调用
    ...
    diagnoseAllShellScripts();
    // 退出时清理
    context.subscriptions.push(formatProvider, codeActionProvider, formatCommand, fixAllCommand, saveListener, openListener, changeListener, configChangeListener, diagnosticCollection, outputChannel);
}
// 防抖定时器
let debounceTimer;
...
class ShellFormatCodeActionProvider {
    provideCodeActions(document, range, context, token) {
        ...
        if (context.diagnostics) {
            for (const diagnostic of context.diagnostics) {
                if (diagnostic.source === diagnosticSource) {
                    const fixThisAction = new vscode.CodeAction('Fix this issue with shfmt', vscode.CodeActionKind.QuickFix);
        ...
        // 修复所有问题的快速修复
        const fixAllAction = new vscode.CodeAction('Fix all with shfmt', vscode.CodeActionKind.SourceFixAll);
...
function buildShfmtArgs(document, config) {
    const defaultArgs = [
        '-i', // 设置缩进空格数（默认：0)
        '2', // 缩进级别
        '-bn', // 二元运算符后换行
    ...
    const args = [];
    // 获取用户自定义参数（字符串格式）
    const userFlag = config.get('flag');
    const userArgs = config.get('args');
    if (userFlag) {
...
async function diagnoseDocument(document) {
    ...
    // 获取配置信息和错误处理方式
    const config = vscode.workspace.getConfiguration('shellformat');
    const onError = config.get('onError', 'showProblem');
...
async function checkWithShellcheck(document) {
    ...
    return new Promise((resolve) => {
        ...
        const shellcheck = (0, child_process_1.spawn)('shellcheck', ['-f', 'gcc', document.fileName]);
        let stdout = [];
        let stderr = [];
    ...
    for (const line of lines) {
        ...
        if (errorMatch) {
            ...
            if (lineNumber >= 0 && lineNumber < document.lineCount) {
                ...
                const diagnostic = new vscode.Diagnostic(lineRange, `${scCode}: ${message}`, type === 'error' ? vscode.DiagnosticSeverity.Error :
 ...
 * 使用 shfmt 进行格式检查
 */
async function checkFormat(document, config) {
    const fileName = path.basename(document.fileName);
    const diagnostics = [];
    return new Promise((resolve) => {
...
function parseShfmtFormatError(document, message) {
    ...
    for (const line of lines) {
        ...
        if (line.startsWith('-') || line.startsWith('+')) {
            ...
            const diagnostic = new vscode.Diagnostic(range, 'Shell script has formatting issues. Run "Fix all with shfmt" to fix.', vscode.DiagnosticSeverity.Warning);
 ...
 */
function parseDiagnostics(document, message) {
